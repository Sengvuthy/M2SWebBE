...
public class ProductServiceImpl implements ProductService {
	...
	public Product save(ProductDTO dto) {
		...
		log.info("✅ Saved product '{}' with category '{}' and supplier '{}'", saved.getName(), saved.getCategoryName(),
	...
	public Page<Product> getProducts(String name, Pageable pageable) {
		...
		return productRepository.findByNameContainingIgnoreCaseOrKhmerNameContainingIgnoreCase(keyword, keyword,
				pageable);
	}
	...
	public Product updateByBarcode(String barcode, ProductDTO dto) {
		...
		if (salePriceChanged) {
			...
			log.info("🔁 Synced sale price to {} import history entries for '{}'", histories.size(), product.getName());
		}
	...
	public void validateStock(Long productId, BigDecimal numberOfUnit) {
		...
		if (product.getAvailableUnit() == null || product.getAvailableUnit().compareTo(numberOfUnit) < 0) {
			throw new API_Exception(HttpStatus.BAD_REQUEST, "Insufficient stock for product: " + product.getName());
		}
	}
	...
	private void recordInitialExpense(Product product, ProductDTO dto) {
		...
		if (unit == null || unit.compareTo(BigDecimal.ZERO) <= 0 || price == null
				|| price.compareTo(BigDecimal.ZERO) < 0) {
			log.info("ℹ️ Skipping expense report for product '{}' due to missing or invalid unit/price",
					product.getName());
			return;
		}
		...
		List<ExpenseReport> existing = expenseReportRepository.findByProductNameAndSource(product.getName(),
				"Creation Expense");
		...
		log.info(" {} creation expense for product '{}': {} units × {} = {} (source={})", action, product.getName(),
				unit, price, totalAmount, report.getSource());
	}
